...
describe('Login', () => {
    ...
    it('Login', () => {
        cy.intercept('GET', '**/public/getProductDetails/67ca29e06d232b1312a9cd5b', {fixture: 'checkout.json'}).as('intercept')
        cy.get('[data-testid="search-icon"]').click()
        cy.get('[data-testid="searchInput"]').click().type('Camiseta EBAC')
        cy.get('[data-testid="productDetails"]').contains('Camiseta').click({force: true})
        cy.get('[data-testid="addToCart"]').click()
        cy.get('[data-testid="productName"]').first().should('have.text', 'Camiseta EBAC')
        })
    })